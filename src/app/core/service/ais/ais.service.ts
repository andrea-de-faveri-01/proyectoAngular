...
export class AisService {
  ...
  constructor(
    private apiAisService : ApiAisService
  ) { }
  ...
  return this.apiAisService.createApiAi(body).pipe(
  map((apiAi) => this.transformAi(apiAi))
  );
  }

  public editAi(body: AiRequestBody, _id: string): Observable<AiI> {
    return this.apiAisService.editApiAi(body, _id).pipe(
      map((apiAi) => this.transformAi(apiAi))
    );
  }

  public deleteAi(_id: string): Observable<AiI> {
    return this.apiAisService.deleteApiAi(_id).pipe(
      map((apiAi) => this.transformAi(apiAi))
    );
  }
